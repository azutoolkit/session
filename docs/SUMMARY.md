# Table of Contents

## Getting Started

* [Introduction](README.md)
* [Installation](getting-started/installation.md)
* [Quick Start](getting-started/quick-start.md)

## Configuration

* [Basic Configuration](configuration/basic.md)
* [Security Settings](configuration/security.md)
* [Performance Settings](configuration/performance.md)

## Architecture & Improvements

* [Overview](architecture/overview.md)
* [Phase 1: Code Quality](architecture/phase-1-code-quality.md)
* [Phase 2: Developer Experience](architecture/phase-2-developer-experience.md)
* [Phase 3: Store Consolidation](architecture/phase-3-store-consolidation.md)
* [Phase 4: Provider Simplification](architecture/phase-4-provider-simplification.md)

## Storage Backends

* [Overview](storage-backends/overview.md)
* [Cookie Store](storage-backends/cookie-store.md)
* [Memory Store](storage-backends/memory-store.md)
* [Redis Store](storage-backends/redis-store.md)
* [Clustered Redis Store](storage-backends/clustered-redis-store.md)

## Clustering

* [Overview](clustering/overview.md)
* [Configuration](clustering/configuration.md)
* [Local Cache](clustering/local-cache.md)
* [Multi-Node Setup](clustering/multi-node-setup.md)

## Core Features

* [Type-Safe Sessions](features/type-safe-sessions.md)
* [Flash Messages](features/flash-messages.md)
* [Session Lifecycle](features/session-lifecycle.md)

## Security

* [Encryption & Signing](security/encryption.md)
* [Key Derivation](security/key-derivation.md)
* [Client Binding](security/client-binding.md)

## Resilience

* [Circuit Breaker](resilience/circuit-breaker.md)
* [Retry Logic](resilience/retry-logic.md)
* [Error Handling](resilience/error-handling.md)

## Observability

* [Metrics](observability/metrics.md)
* [Query Interface](observability/query-interface.md)

## Integrations

* [AZU Framework](integrations/azu-framework.md)
* [HTTP::Server](integrations/http-server.md)

## API Reference

* [Provider API](api/provider.md)
* [Session API](api/session.md)
* [Store API](api/store.md)
* [Exceptions](api/exceptions.md)
